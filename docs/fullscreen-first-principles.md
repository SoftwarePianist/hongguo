...
`docs/fullscreen-controls-handed-back.png` —— 左：播放中点一下屏幕（宿主显示控制层，锁屏/亮度可见）；
右：**同一位置 6 秒后**（宿主自己收回，模块全程零介入）。

![交还宿主后：点一下显示，6 秒后宿主自动收回](fullscreen-controls-handed-back.png)
